...
idenanumber="$2"
idenakeystore="$3"
sed -n "1{p;q}" api.txt >> $idenahome.txt 
portRpc=$(tail -1 $idenanumber-rpc.txt)
portIpf=$(tail -1 $idenanumber-ipf.txt)
sed -i "1d" api.txt
if [ ! -d /home/$idenahome/$idenanumber ]
then
...
User=root
ExecStart=-/usr/bin/idena --profile=lowpower --rpcaddr=localhost --rpcport=
